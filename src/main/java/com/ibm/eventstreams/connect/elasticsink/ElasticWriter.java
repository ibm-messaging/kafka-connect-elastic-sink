...
public class ElasticWriter {
    ...
    public void configure(Map<String, String> props) throws ConnectException {
        ...
        String keyStoreTypeStr = props.get(ElasticSinkConnector.CONFIG_NAME_ES_TLS_KEYSTORE_TYPE);
        if (keyStoreTypeStr != null && !keyStoreTypeStr.isEmpty()) {
            keyStoreType = keyStoreTypeStr;
        }
        ...
        String trustStoreTypeStr = props.get(ElasticSinkConnector.CONFIG_NAME_ES_TLS_TRUSTSTORE_TYPE);
        if (trustStoreTypeStr != null && !trustStoreTypeStr.isEmpty()) {
            trustStoreType = trustStoreTypeStr;
        }

        // Get the SSL verification skip setting
        String skipVerificationStr = props.get(ElasticSinkConnector.CONFIG_NAME_ES_TLS_SKIP_VERIFICATION);
        if (skipVerificationStr != null && !skipVerificationStr.isEmpty()) {
            skipSslVerification = Boolean.parseBoolean(skipVerificationStr);
            if (skipSslVerification) {
                log.warn("SSL verification is disabled. This is not recommended for production environments.");
            }
        }
    ...
    private HttpClient setupConnection() {
        ...
        if (notNullOrEmpty(trustStore)) {
            ...
            log.debug("Using truststore type: {}", trustStoreType);
            if (notNullOrEmpty(trustStorePassword)) {
                sslContextFactory.setTrustStorePassword(trustStorePassword);
            }
        }

        // Apply SSL verification skip setting if enabled
        if (skipSslVerification) {
            log.debug("SSL verification is disabled for Elasticsearch connection");
